...
const emailCustomer = async (order, charge) => {
  const to = 'tim917@gmail.com';
  const sender = `Excited Tim <tim@${config.mailgun.domain}>`;
  const subject = 'In app test';
  const text = 'testing in app';
  const params = helpers.buildQuery({ to, from: sender, text, subject });
  ...
  const result = await helpers.promisifiedHttpsRequest(requestDetails);
